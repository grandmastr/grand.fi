...
test_with_synpress.describe('Wallet Connection Tests', () => {
  test_with_synpress('should connect wallet and show account address', async ({ page, metamask }) => {
    await page.goto('/');
    console.log('Application loaded');
    await page.waitForLoadState('networkidle');
    
    await page.screenshot({ path: 'tests/screenshots/metamask-test-initial.png' });
    ...
    await page.screenshot({ path: 'tests/screenshots/metamask-test-modal.png' });
    ...
    try {
      ...
      await page.screenshot({ path: 'tests/screenshots/metamask-test-connected.png' });
      ...
      if (await addressDisplay.count() > 0) {
        ...
        const updatedButton = page.getByRole('button').filter({ hasText: /Connect|Connected|Account|Wallet/i }).first();
        ...
        await expect(updatedButton).not.toContainText('Connect Wallet', { timeout: 5000 });
      }
    } catch (error) {
      console.error('Error during wallet connection:', error);
      await page.screenshot({ path: 'tests/screenshots/metamask-test-error.png' });
      throw error;
    }
  });
}); 