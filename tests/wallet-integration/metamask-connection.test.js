...
test.describe('MetaMask Wallet Connection', () => {
  test('should simulate wallet connection flow', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/after-simulated-connection.png' });
    ...
    try {
      ...
      if (!hasAddress) {
        await page.getByRole('button').filter({ hasText: /Connect|Account|Wallet|0x/i }).count();
      }
    } catch (error) {
